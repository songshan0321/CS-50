...
def sell():
    ...
    if request.method == "POST":
        ...
        stock = db.execute("SELECT SUM(shares) FROM portfolio WHERE id=:id and symbol=:symbol GROUP BY symbol",id=session["user_id"], symbol=request.form.get("symbol"))

        if int(request.form.get("shares")) > stock[0]["SUM(shares)"]:
